...
using static WalkerAliasRandomizer.WeightErrorHandlingType;
...
namespace WalkerAliasRandomizer
{
    ...
    public class WeightedList<T> : IEnumerable<T>
    {
        ...
        //     float nextProbability = _rand.Next(_totalWeight);
        ...
        public List<float> Weight => _weights;
        public List<int> Alias => _alias;
        public List<float> Probabilities => _probabilities;
        ...
        private void Recalculate()
        {
            ...
            List<float> scaledProbabilityNumerator = new List<float>(Count);
            ...
            Debug.Log(_totalWeight);

            Debug.Log(_minWeight + " " + _maxWeight);
            ...
            while (small.Count > 0 && large.Count > 0)
            {
                ...
                _alias[l] = g; // 5.4
                float tmp = scaledProbabilityNumerator[g] + scaledProbabilityNumerator[l] - _totalWeight; // 5.5, even though using ints for this algorithm is stable
        ...
        private float FixWeight(float weight) => (BadWeightErrorHandling == ThrowExceptionOnAdd) ? FixWeightExceptionOnAdd(weight) : FixWeightSetToOne(weight);
    }
