...
// [ExecuteInEditMode]
public class MisRenderer : RayTracingManager
{
    ...
    public MisRenderer(RayTracingManagerAsset asset, bool EnableAccumulate) : base(asset) {
        ...
        try
        {
            if (frameIndex < 50000)
            {
                using (new ProfilingScope(cmd, new ProfilingSampler("RayTracing")))
                {
                    ...
                    cmd.DispatchRays(rtShader, "MISRayGenShader", (uint)outputTarget.rt.width, (uint)outputTarget.rt.height, 1, camera);
                }
